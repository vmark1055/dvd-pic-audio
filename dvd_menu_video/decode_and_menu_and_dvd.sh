...
if ! [ -d "$VIDEO" ] ;
then
    echo "$MESSAGE_TXT $STEP video's dir don't exists!";
    exit;
else 
    echo "$MESSAGE_TXT $STEP video's dir exists.";
fi
...
do
	    ...
	    ffmpeg  -i "$i" -threads 4 -target pal-dvd "$NameVob";
done; 
vobFiles=$c;
...
echo "$MESSAGE_TXT $STEP created menu's audio from '$i'"
done;
